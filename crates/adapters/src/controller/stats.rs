...
impl CompletionToken {
    ...
    pub fn decode(token: &str) -> Result<Self, anyhow::Error> {
        ...
        match serde_json_path_to_error::from_slice::<ApiCompletionToken>(&json) {
            Ok(api) => Ok(Self {
                incarnation: api.u,
                endpoint_id: api.e,
                count: api.c,
            }),
            Err(e) => Err(anyhow!("error parsing completion token: {e}")),
        }
    }
}
...
impl InputEndpointMetrics {
    pub fn to_api_type(&self) -> ExternalInputEndpointMetrics {
        ExternalInputEndpointMetrics {
...
impl OutputEndpointMetrics {
    ...
    pub fn snapshot(&self) -> ExternalOutputEndpointMetrics {
        ...
        let mut snapshot1 = self.to_api_type();
        loop {
            let snapshot2 = self.to_api_type();
            if snapshot1 == snapshot2 {
                return snapshot1;
            }
            snapshot1 = self.to_api_type();
            if snapshot1 == snapshot2 {
                return snapshot1;
            }
        }
    ...
    pub fn to_api_type(&self) -> ExternalOutputEndpointMetrics {
        ExternalOutputEndpointMetrics {
            transmitted_records: self.transmitted_records.load(Ordering::Acquire),
            ...
            num_encode_errors: self.num_encode_errors.load(Ordering::Relaxed),
            num_transport_errors: self.num_transport_errors.load(Ordering::Relaxed),
