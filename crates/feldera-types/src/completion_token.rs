...
pub struct CompletionTokenResponse {
    ...
    ///
    /// A token is bound to the pipeline incarnation that issued it. After the
    /// pipeline is suspended and resumed from a checkpoint or restarted, tokens
    /// from the previous incarnation are no longer valid and `/completion_status`
